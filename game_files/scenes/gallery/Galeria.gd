...
func _ready():
	for item in ItemTextureMap.items.values():
		item_list.add_item(tr(item.name), item.texture)
...
func _update_info(i : int):
	...
	if item.type == "REEE":
		item_info_blaster.set_text("""Pode ser usado para fazer as Engenhocas Recicladas""")
		if flag :
			item_info_qtd.set_text(str(item.count) + "/" + str(item_qtd_min) +" coletados")
			calculate_item(item)
		else:
