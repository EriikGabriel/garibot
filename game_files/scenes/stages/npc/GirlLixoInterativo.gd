...
func _ready():
	...
	$GirlBody/Reaction.set_reaction('Thinking')
...
func change_state(state):
	...
	if actual_state == 2:
		$GirlBody/Corpo/PersonAnimationPlayer.play("attack")
	elif actual_state == 0:
		if qtditem == 0:
...
func converter_manager() :
	if actual_state == 1 :
		if min_pessoas <= node_global.get_people_collect():
			...
			garibot_followers.add_person('girl', shirt_color, hair_color)
			queue_free()
		else :
			change_state("VERMELHO")
			throw_enemies()
			$GirlBody/Reaction.set_reaction('Angry')
...
func _on_Area2D_body_entered(body):
	if body.is_in_group("player"):
		...
		garibot_followers.ready_set_person('girl',shirt_color, hair_color)
		interact()
...
func _select_random_color():
	...
	$GirlBody.shirt_color = shirt_color
	$GirlBody.hair_color = hair_color
	$GirlBody.update_colors()
